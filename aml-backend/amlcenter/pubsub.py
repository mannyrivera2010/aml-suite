"""
Dispatcher
"""


class Observer(object):

    def events_to_listen(self):
        "The base classes should funtion should return a list of topics to listen to"
        raise NotImplementedError()

    def execute(self, event_type, **kwargs):
        raise NotImplementedError()


class Dispatcher(object):

    def __init__(self):
        self.observers = {}

    def register(self, observer_class, **kwargs):
        observer_name = observer_class.__name__

        if observer_name not in self.observers:
            if kwargs:
                self.observers[observer_name] = observer_class(**kwargs)  # Making instance of class
            else:
                self.observers[observer_name] = observer_class()  # Making instance of class

    def publish(self, event_type, **kwargs):
        for key in self.observers:
            class_object = self.observers[key]
            if event_type in class_object.events_to_listen():
                class_object.execute(event_type, **kwargs)

                if hasattr(class_object, event_type):  # Check to see if method exist
                    event_method = getattr(class_object, event_type)  # Getting Method
                    if hasattr(event_method, '__self__'):   # Check to see if method is a bounded
                        event_method(**kwargs)


# Import Observers
import amlcenter.auth.observers as auth_observers
import amlcenter.api.listing.observers as listing_observers
import amlcenter.api.bookmark.observers as bookmark_observers

# Dispatcher Instance
dispatcher = Dispatcher()

dispatcher.register(listing_observers.ListingObserver)
dispatcher.register(auth_observers.AuthObserver)
dispatcher.register(bookmark_observers.BookmarkObserver)
