...
CREATE_SERVER_CERT(){
  ...
  openssl x509 -req -days 365 -in $SERVER_REQUEST -passin file:$CA_PASSWORD_FILE -CA $CA_SIGNED_CERT_FILENAME -CAkey $CA_KEY_FILENAME -set_serial 01 -out $SERVER_SIGNED_CERT
  ...
  openssl x509 -req -days 365 -in $SERVER_NOPASS_REQUEST -passin file:$CA_PASSWORD_FILE -CA $CA_SIGNED_CERT_FILENAME -CAkey $CA_KEY_FILENAME -set_serial 01 -out $SERVER_SIGNED_NOPASS_CERT
}
...
CREATE_CLIENT_CERT(){
  ...
  openssl x509 -req -days 365 -in $CURRENT_USER_FILENAME.csr -passin file:$CA_PASSWORD_FILE -CA $CA_SIGNED_CERT_FILENAME -CAkey $CA_KEY_FILENAME -set_serial 01 -out $CURRENT_USER_FILENAME.crt
